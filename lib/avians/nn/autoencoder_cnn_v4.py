...
class AutoencoderDataset():
    ...
    def file_to_write(self, model_key=""):
        sizestr = 'x'.join([str(s) for s in self.component_images[:self.new_component_index].shape])
        ds_filename = os.path.join(save_dir(),
                                   "autoencoder-dataset--{}--size-{}".format(model_key, sizestr))
        return ds_filename

    def write(self, model_key=""):
        np.savez_compressed(self.file_to_write(model_key),
...
def save_and_upload_model(model, model_key=""):
    model_file = 'model-{}-{}'.format(model_key, dt.strftime(dt.now(), '%F-%T'))
    json_model = model.to_json()
    json_filename = model_file + ".json"
    open(json_filename, 'w').write(save_dir() + json_model)
    # accu.save_to_s3(json_filename)
...
def create_word_images(word, fonts=['Amiri', 'Droid Sans Arabic'], dpis=[1200]):
    ...
    for font in fonts:
        for dpi in dpis:
            word_img = auiu.get_gray_word_image(word,
                                                fontname=font,
                                                dpi=dpi,
                                                background=0,
                                                foreground=255)
...
def train_model(dataset,
    ...
    val_data = accu.prepare_validation_data(x_train, x_train)
    ...
    save_and_upload_model(autoencoder, "autoencoder")
    save_and_upload_model(decoder, "decoder")
    save_and_upload_model(encoder, "encoder")
...
def autoencoder_cnn_v2(wordlist_file, size=64, **kwargs):
...
def main():
    ...
    autoencoder_cnn_v2(args['word-list'])
